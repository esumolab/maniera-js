...
const Contact = ({ ...props }) => {
  ...
  const buttonTop = isMobileOrTablet ? (
    <LinkBoxMobile {...buttonPropsTop} />
  ) : (
    <LinkBox {...buttonPropsTop} />
  );
  const buttonBottom = isMobileOrTablet ? (
    <LinkBoxMobile {...buttonPropsBottom} />
  ) : (
    <LinkBox {...buttonPropsBottom} />
  );
        ...
        <Fade bottom>
          <div className="contact-box contact-info">
            ...
            <p className="contact-info__address">
              Mokotowska 39, 00-551 Warszawa
            </p>
            <p className="contact-info__phone">
              {trans.book}: +48 787 00 39 39
            </p>
            {buttonTop}
          </div>
        </Fade>
